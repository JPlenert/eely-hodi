...
** link:https://developer.tibber.com/[Tibber]
...
== Preisanzeige (dynamische Stromtarife)

Die Preisanzeige kann die dynamischen Preise von dem Stromanbieter Tibber anzeigen. Damit die korrekten Preise angezeigt werden können, muss ein Vertrag mit Tibber zur Stromlieferung abgeschlossen sein. Anschließend muss ein API-Token über die link:https://developer.tibber.com/settings/accesstoken[Seite von Tibber] erzeugt werden.

.Ansicht Hodi mit Preisanzeige
image::hodi_tibber_prices.png["Ansicht Hodi mit Preisanzeige", width = 350]

HoDi zeigt dabei immer drei Preise in Cents an: Den in dieser Stunde aktuellen Preis, sowie die Preise der nächsten zwei Stunden. Die Preise können in grüner, gelber oder roter Schrift angezeigt werden - je nach PriceRating von Tibber.

Zur Parametrierung sind folgende Schritte notwending:

* Aktivieren von `Show energy prices`
* `Tibber cloud token` eintragen
* `Devices config` hinzufügen eines Gerätes `Tibber Prices`
